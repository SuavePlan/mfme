...
static inline void ValidCtrCheck(TVideo *)
{
    new TVideo(NULL);
}
//---------------------------------------------------------------------------
__fastcall TVideo::TVideo(TComponent* Owner)
    : TImage32(Owner)
{
  Bitmap->Clear();
...
//---------------------------------------------------------------------------
__fastcall TVideo::~TVideo(void)
{
  delete shadow;
}
//---------------------------------------------------------------------------

void __fastcall TVideo::Loaded(void)
{
  Invalidate();
}
//---------------------------------------------------------------------------

void __fastcall TVideo::Paint(void)
{
  BeginUpdate();
  Bitmap->Assign(shadow);
  EndUpdate();
  TImage32::Paint();
}
//---------------------------------------------------------------------------
void __fastcall TVideo::WriteMemory(Byte *map)
{
  memory = map;
}
//---------------------------------------------------------------------------
void __fastcall TVideo::Dump()
{
  shadow->SaveToFile("dump.bmp");
}
//---------------------------------------------------------------------------
void __fastcall TVideo::DrawRow(int row)
{
int x,y;
int pos = 64 * row;
Byte dbl;
...
void __fastcall TVideo::SetColour(int nbr, TColor colour)
{
  colours[nbr] = Color32(colour);
}
//---------------------------------------------------------------------------

TColor __fastcall TVideo::GetColour(int nbr)
{
  return WinColor(colours[nbr]);
}
//---------------------------------------------------------------------------

void __fastcall TVideo::WriteRowTable(Word *ptr)
{
  rowtable = ptr;
}
//---------------------------------------------------------------------------
void __fastcall TVideo::WritePalette(Byte *ptr)
{
  for ( int i = 0; i < 16; i++ ) {
...
//---------------------------------------------------------------------------
void __fastcall TVideo::DisplayChar(int x, int y, int pos, Byte dbl)
{
int i, j;
Byte value;
Byte value1;
