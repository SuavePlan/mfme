...
{$ENDIF}
  ...
  public
    ...
    procedure Flush(const SrcRect: TRect); overload; virtual;
  end;
...
//  R := Canvas.ClipRect; // My Changes
  with Result do
  begin
    // by default, the whole control is buffered
    Left := 0; //R.Left;  // = 0
    Top := 0; //R.Top;    // = 0
    Right := Width;  //R.Right; // = Width
    Bottom := Height;  //R.Bottom;   // = Height
  end;
end;
...
procedure TCustomImage32.Flush;
begin
  if (FBitmap.Handle <> 0) and (FBuffer.Handle <> 0) then
  begin
      FBitmap.Lock;
      try
//        with GetViewportRect do
            FBitmap.DrawTo(FBuffer, GetViewportRect);
      finally
        FBitmap.Unlock;
      end;
  end;
...
procedure TCustomImage32.Flush(const SrcRect: TRect);
var
  DstRect: TRect;
//  h: single;
begin
  ...
  begin
      ...
      try
        DstRect := SrcRect;
        if ( Width = FBitmap.Width ) and ( Height = FBitmap.Height ) then
          with SrcRect do
            BitBlt(FBuffer.Handle, Left, Top, Right - Left, Bottom - Top,
              FBitmap.Handle, Left, Top, SRCCOPY)
        else
          begin
            DstRect.Left := Ceil(SrcRect.Left * Width / FBitmap.Width);
            DstRect.Right := Floor(SrcRect.RIght * Width / FBitmap.Width);
            DstRect.Top := Ceil(SrcRect.Top * Height / FBitmap.Height);
            DstRect.Bottom := Floor(SrcRect.Bottom  * Height / FBitmap.Height);
//            h := MulDiv(Width, $FFFF, FBitmap.Width);
//            DstRect.Left := Ceil( (h * SrcRect.Left) / 65535);
//            DstRect.Right := Floor( (h * SrcRect.Right) / 65535);
//            h :=  MulDiv(Height, $FFFF, FBitmap.Height);
//            DstRect.Top := Ceil((h * SrcRect.Top) /65535);
//            DstRect.Bottom := Floor((h * SrcRect.Bottom) /65535);
            FBitmap.DrawTo(FBuffer, DstRect, SrcRect);
          end;
      finally
        FBitmap.Unlock;
      end;
  end;
inherited Flush(DstRect);
end;
