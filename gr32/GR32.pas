...
{*********************************************}
// $Id: GR32.pas,v 1.5 2002/03/27 20:19:04 alex Exp $
...
function MakeRectF(const FR: TFloatRect; Rounding: TRectRounding = rrClosest): TRect;// overload;
function MakeRectX(const FXR: TFixedRect; Rounding: TRectRounding = rrClosest): TRect;// overload;
...
function MakeRectF(const FR: TFloatRect; Rounding: TRectRounding): TRect;
begin
  with FR do
    case Rounding of
      rrClosest:
...
function MakeRectX(const FXR: TFixedRect; Rounding: TRectRounding): TRect;
begin
  with FXR do
    case Rounding of
      rrClosest:
...
begin
  ...
  BlockTransfer(Dst, 0, 0, Self, MakeRect(0, 0, Width, Height), DrawMode, FOnPixelCombine);
  Dst.Changed;
end;
...
begin
  if Empty or Dst.Empty then Exit;
  BlockTransfer(Dst, DstX, DstY, Self, MakeRect(0, 0, Width, Height), DrawMode, FOnPixelCombine);
  Dst.Changed;
end;
...
begin
  if Empty or Dst.Empty then Exit;
  StretchTransfer(Dst, DstRect, Self, MakeRect(0, 0, Width, Height),
    StretchFilter, DrawMode, FOnPixelCombine);
  Dst.Changed;
end;
...
begin
  if Empty or Dst.Empty then Exit;
  StretchTransfer(Dst, DstRect, Self, SrcRect, StretchFilter, DrawMode, FOnPixelCombine);
  Dst.Changed;
end;
...
begin
  ...
  begin
    ...
    if R.Top > 0 then FillRect(0, 0, Width - 1, R.Top, FillColor)
