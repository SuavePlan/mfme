...
{*********************************************}
// $Id: GR32_Transforms.pas,v 1.3 2002/03/08 04:15:49 alex Exp $
...
begin
  ...
  SrcX := SrcRect.Left;
  SrcY := SrcRect.Top;
  IntersectRect(S, SrcRect, MakeRect(0, 0, Src.Width, Src.Height));
  OffsetRect(S, DstX - SrcX, DstY - SrcY);
  IntersectRect(D, S, MakeRect(0, 0, Dst.Width, Dst.Height));
...
var
  ...
  SFX, SFY: TFixed;
  SrcP, DstP: PColor32;
  DstX, DstY, SrcX, SrcY, OldSrcY: Integer;
...
begin
  ...
  IntersectRect(R, DstRect, MakeRect(0, 0, Dst.Width, Dst.Height));
  ...
  begin
    ...
    begin
      ...
      begin
        MoveLongword(
          Dst.PixelPtr[R.Left, DstY - 1]^,
          Dst.PixelPtr[R.Left, DstY]^,
          R.Right - R.Left);
      end
      else
      begin
        SrcP := Src.PixelPtr[SrcRect.Left, SrcY]; // source scanline
...
begin
  ...
  // clip SrcRect
  R := MakeRectF(Transformation.SrcRect);
