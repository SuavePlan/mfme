...
#define rate_adjust(x) (uint)((double)(x)*clk/72/rate + 0.5) /* +0.5 for round */
...
static int halfsintable[PG_WIDTH] ;

static int *waveform[2] = {fullsintable,halfsintable} ;
...
/* dB to Liner table */
static int DB2LIN_TABLE[DB_MUTE+1] ;

/* Liner to Log curve conversion table (for Attack rate). */
static uint AR_ADJUST_TABLE[1<<EG_BITS] ;
...
static uint attacktable[16][4] ;
static uint decaytable[16][4] ;
#endif

/* Phase incr table for Attack */
static uint dphaseARTable[16][16] ;
/* Phase incr table for Decay and Release */
static uint dphaseDRTable[16][16] ;

/* KSL + TL Table */
static uint tllTable[16][8][1<<TL_BITS][4] ;
static int rksTable[2][8][2] ;

/* Phase incr table for PG */
static uint dphaseTable[512][8][16] ;
...
****************************************************/
int __fastcall ym2413::Max(int i,int j){

  if(i>j) return i ; else return j ;

}

int __fastcall ym2413::Min(int i,int j){
...
void __fastcall ym2413::makeAdjustTable(void){

  int i ;

  AR_ADJUST_TABLE[0] = (1<<EG_BITS) ;
  for ( i=1 ; i < 128 ; i++)
    AR_ADJUST_TABLE[i] = (uint)((double)(1<<EG_BITS) - 1 - (1<<EG_BITS) * log(i) / log(128)) ;

}


/* Table for dB(0 -- (1<<DB_BITS)) to Liner(0 -- DB2LIN_AMP_WIDTH) */
void __fastcall ym2413::makeDB2LinTable(void){

  int i ;

  for( i=0 ; i < DB_MUTE ; i++)
    DB2LIN_TABLE[i] = (int)((double)(1<<DB2LIN_AMP_BITS) * pow(10,-(double)i*DB_STEP/20)) ;
...
int __fastcall ym2413::lin2db(double d){

  if(d == 0) return (1<<DB_BITS) - 1 ;
  else return (int)(-((int)((double)20*log10(d)/SIN_STEP)) * (SIN_STEP/DB_STEP)) ;
 ...
 */
void __fastcall ym2413::makeSinTable(void){

  int i ;

  for( i = 0 ; i < PG_WIDTH/4 ; i++ )
    fullsintable[i] = lin2db(sin(2.0*M_PI*i/PG_WIDTH)) + 1 ;
...
void __fastcall ym2413::makePmTable(void){

  int i ;

  for(i = 0 ; i < PM_PG_WIDTH ; i++ )
    pmtable[i] = (int)((double)PM_AMP * pow(2,(double)PM_DEPTH*sin(2.0*M_PI*i/PM_PG_WIDTH)/1200)) ;

}

/* Table for Amp Modulator */
void __fastcall ym2413::makeAmTable(void){

  int i ;

  for(i = 0 ; i < AM_PG_WIDTH ; i++ )
    amtable[i] = (int)((double)AM_DEPTH/2/DB_STEP * (1.0 + sin(2.0*M_PI*i/PM_PG_WIDTH))) ;

}

/* Phase increment counter table */
void __fastcall ym2413::makeDphaseTable(void){

  uint fnum, block , ML ;
  uint mltable[16]={ 1,1*2,2*2,3*2,4*2,5*2,6*2,7*2,8*2,9*2,10*2,10*2,12*2,12*2,15*2,15*2 } ;
...
void __fastcall ym2413::makeTllTalbe(void){

#if DB_BITS == 7
#define dB2(x) (uint)(((uint)(x/0.375)) * 0.375 * 2)
#else
#define dB2(x) (uint)((x)*2)
#endif

	static uint kltable[16] = {
        ...
        for(KL=0; KL<4; KL++){
          ...
          if(KL==0){
            ...
            if(tmp <= 0)
              ...
              tllTable[fnum][block][TL][KL] = (uint)((tmp >> ( 3 - KL ))/DB_STEP) + EXPAND_BITS(TL,TL_BITS,DB_BITS) ;
          }
...
void __fastcall ym2413::makeDphaseARTable(void){
...
void __fastcall ym2413::makeDphaseDRTable(void){
...
void __fastcall ym2413::makeRksTable(void){
...
void __fastcall ym2413::dump2patch(unsigned char *dump, OPLL_PATCH *patch){
...
void __fastcall ym2413::makeDefaultPatch(void){
...
uint __fastcall ym2413::calc_eg_dphase(OPLL_SLOT *slot){
...
void __fastcall ym2413::OPLL_forceRefresh(OPLL *opll){
...
void __fastcall ym2413::refresh(OPLL_SLOT *slot){
...
/* Slot key on  */
void __fastcall ym2413::slotOn(OPLL_SLOT *slot){
...
/* Slot key off */
void __fastcall ym2413::slotOff(OPLL_SLOT *slot){
...
/* Channel key on */
void __fastcall ym2413::keyOn(OPLL *opll, int i){
...
/* Channel key off */
void __fastcall ym2413::keyOff(OPLL *opll, int i){
...
void __fastcall ym2413::keyOn_BD(OPLL *opll){
...
void __fastcall ym2413::keyOn_SD(OPLL *opll){
  if(!opll->slot_on_flag[SLOT_SD]){
    slotOn(opll->CAR(7)) ;
    opll->debug_rythm_flag |= 8 ;
  }
}
void __fastcall ym2413::keyOn_TOM(OPLL *opll){
  if(!opll->slot_on_flag[SLOT_TOM]){
    slotOn(opll->MOD(8)) ;
    opll->debug_rythm_flag |= 4 ;
  }
}
void __fastcall ym2413::keyOn_HH(OPLL *opll){
  if(!opll->slot_on_flag[SLOT_HH]){
    slotOn(opll->MOD(7)) ;
    opll->debug_rythm_flag |= 2 ;
  }
}
void __fastcall ym2413::keyOn_CYM(OPLL *opll){
...
#else
void __fastcall ym2413::keyOn_BD(OPLL *opll){ keyOn(opll,6) ; }
void __fastcall ym2413::keyOn_SD(OPLL *opll){ if(!opll->slot_on_flag[SLOT_SD]) slotOn(opll->CAR(7)) ; }
void __fastcall ym2413::keyOn_TOM(OPLL *opll){ if(!opll->slot_on_flag[SLOT_TOM]) slotOn(opll->MOD(8)) ; }
void __fastcall ym2413::keyOn_HH(OPLL *opll){ if(!opll->slot_on_flag[SLOT_HH]) slotOn(opll->MOD(7)) ; }
void __fastcall ym2413::keyOn_CYM(OPLL *opll){ if(!opll->slot_on_flag[SLOT_CYM]) slotOn(opll->CAR(8)) ; }
#endif

/* Drum key off */
void __fastcall ym2413::keyOff_BD(OPLL *opll){ keyOff(opll,6) ; }
void __fastcall ym2413::keyOff_SD(OPLL *opll){ if(opll->slot_on_flag[SLOT_SD]) slotOff(opll->CAR(7)) ; }
void __fastcall ym2413::keyOff_TOM(OPLL *opll){ if(opll->slot_on_flag[SLOT_TOM]) slotOff(opll->MOD(8)) ; }
void __fastcall ym2413::keyOff_HH(OPLL *opll){ if(opll->slot_on_flag[SLOT_HH]) slotOff(opll->MOD(7)) ; }
void __fastcall ym2413::keyOff_CYM(OPLL *opll){ if(opll->slot_on_flag[SLOT_CYM]) slotOff(opll->CAR(8)) ; }

/* Change a voice */
void __fastcall ym2413::setPatch(OPLL *opll, int i, int num){
...
void __fastcall ym2413::setSlotPatch(OPLL_SLOT *slot, OPLL_PATCH *patch){
...
void __fastcall ym2413::setSustine(OPLL *opll, int c, int sustine){
...
*/
void __fastcall ym2413::setVolume(OPLL *opll, int c, int volume){
...
void __fastcall ym2413::setSlotVolume(OPLL_SLOT *slot, int volume){
...
void __fastcall ym2413::setVolume_BD(OPLL *opll, int volume){ setVolume(opll,6,volume) ; }
void __fastcall ym2413::setVolume_HH(OPLL *opll, int volume){ setSlotVolume(opll->MOD(7),volume) ; }
void __fastcall ym2413::setVolume_SD(OPLL *opll, int volume){ setSlotVolume(opll->CAR(7),volume) ; }
void __fastcall ym2413::setVolume_TOM(OPLL *opll, int volume){ setSlotVolume(opll->MOD(8),volume) ; }
void __fastcall ym2413::setVolume_CYM(OPLL *opll, int volume){ setSlotVolume(opll->CAR(8),volume) ; }

/* Set F-Number ( fnum : 9bit ) */
void __fastcall ym2413::setFnumber(OPLL *opll, int c, int fnum){
...
void __fastcall ym2413::setBlock(OPLL *opll, int c, int block){
...
/* Change Rythm Mode */
void __fastcall ym2413::setRythmMode(OPLL *opll, int mode){
...
void __fastcall ym2413::OPLL_copyPatch(OPLL *opll, int num, OPLL_PATCH *patch){
...
void __fastcall ym2413::OPLL_SLOT_reset(OPLL_SLOT *slot){
...
OPLL_SLOT * __fastcall ym2413::OPLL_SLOT_new(void){
...
void __fastcall ym2413::OPLL_SLOT_delete(OPLL_SLOT *slot){

  free(slot) ;

}

void __fastcall ym2413::OPLL_CH_reset(OPLL_CH *ch){
...
OPLL_CH * __fastcall ym2413::OPLL_CH_new(void){
...
void __fastcall ym2413::OPLL_CH_delete(OPLL_CH *ch){
...
OPLL *__fastcall ym2413::OPLL_new(void){
  ...
  for( i = 0 ; i < 19 ; i++ ){
    patch[i] = (OPLL_PATCH *)calloc(sizeof(OPLL_PATCH),2) ;
    if(patch[i] == NULL){
...
void __fastcall ym2413::OPLL_delete(OPLL *opll){
...
void __fastcall ym2413::OPLL_reset_patch(OPLL *opll){
...
void __fastcall ym2413::OPLL_reset(OPLL *opll){
  ...
  opll->rythm_mode = 0 ;
  opll->pm_phase = 0 ;
  opll->pm_dphase = (uint)rate_adjust(PM_SPEED * PM_DP_WIDTH / (clk/72) ) ;
  opll->am_phase = 0 ;
  opll->am_dphase = (uint)rate_adjust(AM_SPEED * AM_DP_WIDTH / (clk/72) ) ;
...
void __fastcall ym2413::OPLL_init(int c, int r){
...
uint __fastcall ym2413::mrand(void){
...
/* Update Noise unit */
void __fastcall ym2413::update_noise(OPLL *opll){

  opll->whitenoise = (((uint)(DB_NOISE/DB_STEP)) * mrand()) >> 16 ;

}

/* Update AM, PM unit */
void __fastcall ym2413::update_ampm(OPLL *opll){
...
/* PG */
uint __fastcall ym2413::calc_phase(OPLL_SLOT *slot, int lfo_pm){
...
/* EG */
uint __fastcall ym2413::calc_envelope(OPLL_SLOT *slot, uint lfo_am){

  #define SL(x) EXPAND_BITS((uint)(x/DB_STEP),DB_BITS,EG_DP_BITS)
  static uint SLtable[16] = {
...
int __fastcall ym2413::calc_slot_car(OPLL_SLOT *slot, int fm){
...
int __fastcall ym2413::calc_slot_mod(OPLL_SLOT *slot){
...
int __fastcall ym2413::calc_slot_tom(OPLL_SLOT *slot){
...
/* calc SNARE slot */
int __fastcall ym2413::calc_slot_snare(OPLL_SLOT *slot, uint whitenoise){
...
int __fastcall ym2413::calc_slot_hat(OPLL_SLOT *slot, int fm, int whitenoise){
...
int16 __fastcall ym2413::OPLL_calc(OPLL *opll){
...
*****************************************************/
void __fastcall ym2413::OPLL_writeReg(OPLL *opll, uint reg, uint data){
...
void ym2413::Update(short *buffer, int length, uint clks)
{
	while (length--) *buffer++ = OPLL_calc(opll);
}

__fastcall ym2413::ym2413(void)
{
	OPLL_init(3579545,64000);
//	OPLL_init(8000000,64000);
    opll = OPLL_new();
    reset();
}

void __fastcall ym2413::stop (void)
{
  OPLL_delete(opll);
}

void __fastcall ym2413::reset (void)
{
  OPLL_reset(opll);
  OPLL_reset_patch(opll);
}

void __fastcall ym2413::write_reg(int data)
{
	ym_latch = data;
}

void __fastcall ym2413::write_data(int data)
{
	OPLL_writeReg(opll, ym_latch, data);
}
